...
exports.mustache = new Transformer({
  ...
  outputFormat: 'xml',
  sync: function(str, options){
    var tmpl = this.cache(options) || this.cache(options, {tokens: this.engine.parse(str), template: str});
    return this.engine.render(tmpl.template, options, options.partials);
  }
});
...
exports.dust = new Transformer({
  ...
  async: function (str, options, cb) {
    ...
    var tmpl = this.cache(options) || this.cache(options, this.engine.compileFn(str, null));
